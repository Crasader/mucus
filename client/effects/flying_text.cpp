...
namespace effects
{
    std::weak_ptr<FlyingText> FlyingText::create(const pr::Vec2 &position, cc::Node *parent, const char *text, size_t size, const cc::ccColor3B &color, float duration)
    {
        auto effect = std::make_shared<FlyingText>();

        effect->m_position = position;

        effect->m_label = cc::LabelTTF::create(text, "", size);
